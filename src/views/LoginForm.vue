    ...
    <form class="mt-8 space-y-6" @submit.prevent="onSubmit">
      <div class="space-y-4">
        <div>
          ...
          <input id="username" name="username" type="text" v-model="username" required class="auth-input" />
        </div>

        <div>
          <label for="password" class="auth-label">{{ t('auth.login.password') }}</label>
          <input id="password" name="password" type="password" v-model="password" required class="auth-input" />
        </div>
      </div>

      <div class="flex items-center justify-between">
        <label class="flex items-center gap-2 auth-check-label">
          <input type="checkbox" class="auth-checkbox" />
          {{ t('auth.login.remember') }}
        </label>
        <div class="text-sm">
          <router-link to="/auth/forgot" class="auth-link">
            {{ t('auth.login.forgot') }}
          </router-link>
        </div>
      </div>
      ...
      <div class="relative mt-6">
        <div class="absolute inset-0 flex items-center">
          <div class="w-full auth-sep-line"></div>
        </div>
